...
static void __free_changed_cb_bd(gpointer data)
{
	struct _badge_cb_data *bd = (struct _badge_cb_data *)data;
	if (bd)
		free(bd);
}
...
static void __free_changed_cb_hash(gpointer data)
{
	...
	if (changed_cb_list)
		g_list_free_full(changed_cb_list, __free_changed_cb_bd);
}
...
int _badge_register_changed_cb(badge_change_cb callback, void *data, uid_t uid)
{
	...
	if (_badge_cb_hash == NULL)
		_badge_cb_hash = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL, __free_changed_cb_hash);
